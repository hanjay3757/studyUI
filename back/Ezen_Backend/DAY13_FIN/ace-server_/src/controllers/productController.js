...
exports.getAllProducts = async (req, res) => {
  ...
  // const rawOrder = decodeURIComponent(req.query.order || '');
  const order = orderMapping[rawOrder] || ['id', 'DESC'];
...
exports.getAllProductsPaging = async (req, res) => {
  ...
  const rawOrder = decodeURIComponent(req.query.order || '');
  const order = allowedOrders[rawOrder] || ['id', 'DESC'];

  const page = parseInt(req.query.page) || 1;  // 기본 1페이지
  const limit = parseInt(req.query.limit) || 10; // 한 페이지당 10개
...
exports.getProductBySpec = async (req, res) => {
  ...
  try {
    ...
    console.error('스펙별 상품 조회 오류:', err);
    res.status(500).json({ message: '서버 오류' });
  }
};//----------------------------
//api/products/1
